...
setApiKeys() {
  ...
  render() {
  	...
  	return (
		...
		<div id="api-keys" className={`api-keys-form-wrapper ${this.props.shown}`}>
			<div className="api-keys-input-wrapper shadowed">
			  <span>
			    <input ref="apiKey" id="apiKey" placeholder={`Api key: ${apiKeys.key}`}/> 
			  </span>
			  <span>
			    <input ref="apiSecret" id="apiSecret" placeholder={`Api secret: ${apiKeys.secret}`}/> 
			  </span>			   
			</div>
			<div className="api-keys-confirm-buttons">  
			  <button id="confirm" className="primary" onClick={() => this.setApiKeys()}>✔</button>
			  <button id="clear" onClick={() => this.props.hideApiKeys()}>✖</button>
			</div>
		</div>
