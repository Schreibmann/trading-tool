  ...
  render() {
    ...
    return (
      ...
      <div id={`graph-item__${this.props.id}`} className="graph-item app-block__item__elem">
        <span className="close shadowed" title="Close" onClick={() => this.props.close()}></span>
        <span className="pair-info">
          <strong>
            {`[${this.props.pair}] `}
          </strong>
          {`Average price (${this.props.avgPricePeriod} min): `}
          <span className='blue-text'>buy</span>{` ${this.props.buyAvgPrice}${this.props.symbol}, `}
          <span className='red-text'>sell</span>{` ${this.props.sellAvgPrice}${this.props.symbol} `}
        </span>
        <span className="refresh">
         ...
         <iframe title={`graph-frame__${this.props.id}`} 
