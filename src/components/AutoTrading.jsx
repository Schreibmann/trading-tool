...
/*
  ...
  toggleTrading() {
    if (this.state.trading && this.botInterval) {
        ...
        this.botInterval = setInterval(() => this.trade(this.props.currency, this.props.crypto, this.props.pair), 3000);
            ...
            .then((res) => {
              ...
              .then((data) => {
                ...
                if (data.hasOwnProperty('error')) {
                      this.addLog(`Getting pair info for ${pair} failed with error ${data.error}`);
                    } else {

                        let CURRENCY_1_MIN_QUANTITY = data[pair].min_quantity; //Определяем минимальное кол-во крипты на закупку
                        this.addLog(`${now()}: Success. Min quantity is ${CURRENCY_1_MIN_QUANTITY} ${crypto}`);

                        //Получаем все ордера
                        this.addLog(`Checking opened orders`);
                        let openedOrders = api_query("user_open_orders", {})
                          .then(res=>{
                            let data = JSON.parse(res);
                            if (data.hasOwnProperty('error')) {
                              this.addLog(`Getting opened orders for ${pair} failed with error ${data.error}`);
                            } else return data;
                          },  error => {
                                return error;
                              }
                        ...
                        openedOrders.then(orders => {
                          if (typeof(orders[pair]) !== 'undefined') { //Есть ордера по этой паре
                            this.addLog(`Success. Some orders on ${pair} pair`);
                            let buyOrders = [];
                            for (let order of orders[pair]) {
                              //Есть ли ордера на продажу
                              if (order.type === 'sell') { //Есть ордера на продажу, выходим
                                this.addLog(`Quit. Waiting opened sell orders to be done.`);
                                return 0;  
                              } else { //Запоминаем ордера на покупку
                                buyOrders.push(order);
                              }
                            }

                            //Есть ли ордера на покупку
                            if (buyOrders.length > 0) { //Ордера на покупку есть
                              ...
                              for (let order of buyOrders) { 
                                ...
                                api_query("order_trades", {order_id: order['order_id']}) //Смотрим состояние ордера
                                  .then(res => {
                                    if (res.indexOf('Error 50304') <= -1) { //Частично исполнен, выходим
                                      this.addLog("Quit. Wainig buy orders to be done.");
                                      //return 0;
                                    } else { //Частично не исполнен, смотрим как давно висит
                                        let now = new Date();
                                        let timePassed = now.getUnixTimestamp() - 0*60*60 - order['created'];
                                        if (timePassed > ORDER_LIFE_TIME * 60) { //Давно висит, отменяем
                                          this.addLog("Old order. Trying to cancel...");
                                          api_query("order_cancel", {order_id: order['order_id']})
                                            .then(res=>{
                                              (res.result) ? this.addLog('Success. Order cancelled') : this.addLog(`Cancelling order failed with error ${res.error}`)
                                            },  error => { //Cancel order query error
                                                 return error;
                                            }
                                          );
                                        } else { //Нехай еще повисит
                                            this.addLog(`Quit. Waiting buy order to be done. Passed ${timePassed} seconds since creation.`);
                                        }
                                      }
                                  },  error => { //Order trades query error
                                        return error;
                                      }
                                  );
                              }
                              return 0; //Общий выход
                          ...
                          } else { //Ордеров по выбранной паре нет. Создаем (если можем)
                              this.addLog('No orders. Trying to create some...')
                              api_query("user_info", {})
                                .then(res=>{
                                  let data = JSON.parse(res);
                                  //Есть ли крипта на продажу в количестве больше минимального
                                  if (data.balances[crypto] >= CURRENCY_1_MIN_QUANTITY) { //Есть. Продаем дороже, чем купили, с учетом комисси и навара
                                    let wannaGet = CAN_SPEND + CAN_SPEND * (STOCK_FEE+PROFIT_MARKUP);
                                    this.addLog(`Trying to sell ${data.balances[crypto]}${crypto} and get ${wannaGet}${curr}`);
                                    api_query("order_create", //Создаем ордер
                                      { 
                                        'pair': pair, 
                                      ...
                                      },  error => { //Create sell order query error
                                            return error;
                                          }
                                      );

                                  } else { //Если крипты маловато, пробуем докупить. 
                                      //Есть ли бабло?
                                      if (parseFloat(data.balances[curr]) >= CAN_SPEND) { //Бабло есть. Вычисляем среднюю стоимость за указанный период
                                        let deals = getDeals(pair, 10000, 'buy'); //Список завершенных сделок на покупку
                                        deals.then(data => {
                                          let actualDeals = data.filter(deal => {//Выбираем сделки за указанный период
                                            let now = new Date();
                                            let timePassed = now.getUnixTimestamp() - deal['date'] - 0*60*60; //+- время биржи, если расходится
                                          ...
                                          let avgPrice = prices.reduce((first, next) => first + next, 0)/prices.length;
                                          let myNeedPrice = avgPrice - (avgPrice * (STOCK_FEE+PROFIT_MARKUP)); 
                                          let myAmount = CAN_SPEND/myNeedPrice;

                                          this.addLog(`Trying to buy ${myAmount} ${crypto} for ${myNeedPrice} ${curr}`);
                                          //Допускается ли покупка такого количества крипты?
                                          if (myAmount >= CURRENCY_1_MIN_QUANTITY) { //Допускается
                                            api_query("order_create", {'pair': pair, 'quantity': myAmount, 'price': myNeedPrice, 'type': 'buy'}) //Создаем ордер
                                              .then(res=>{
                                                let data = JSON.parse(res);
                                                  if (data.hasOwnProperty('error')) {
                                                   this.addLog(data.error);
                                              ...
                                              },  error => { //Create buy order query error
                                                    return error;
                                                  }
                                              );
                                          } else { //Маловато будет
                                            this.addLog(`Budget is too low. Can only buy less than ${CURRENCY_1_MIN_QUANTITY} ${crypto}`);
                                            return 0;
                                          }
                                        });

                                      } else { //Бабла нет.
                                          this.addLog(`${now()} Not enough ${curr} to start trading. Exit.`);
                                          return 0;
                                      }
                                  } 
                                },  error => { //User info query error
                                    return error;
                                });
                          } 
                        },  error => { //Opened orders query error
                              return error;
                            }
                        );
                } 
      ...
      <div className="auto-trading__inner">
                  ...
                  <div className="curr-pair app-block__item__elem">
                    ...
                    <div className="curr-pair__values ">
                      ...
                      <select className="curr-pair_select-crypto " id="crypto" name="crypto" value={this.props.crypto} onChange={(event, id) => this.props.setCrypto(event, this.props.id)}>
                        <option value="BTC">BTC</option>
                        <option value="LTC">LTC</option>
                  ...
                  <div className="check-period controls-form__item app-block__item__elem">
                    <div className="check-period__label">
                      <label className="check-period__label-caption">Average price period</label>
                    </div>
                    <div className="check-period__values">
                      <select className="avg-price-period" id="avg-price-period" name="avg-price-period" value={this.props.avgPricePeriod} onChange={(event) => this.props.setAvgPricePeriod(event)}>
                  ...
                  <div className="profit controls-form__item app-block__item__elem">
                    <div className="profit__label">
                      <label className="profit__label-caption">
                        Deal profit
                          <span className="profit-persent-value">
                            {` [0.001 = 0.1%]`}
                          </span>
                      </label>
                    </div>
                    <div className="profit__values">
                      <input type="number" min="0.001" step="0.001" className="profit__input" id="profit__input" name="profit__input" placeholder="0.001"></input>
                    </div>
                  </div> 

                  <div className="max-purch controls-form__item app-block__item__elem">
                    <div className="max-purch__label">
                      <label className="max-purch__label-caption" title="How much currency bot spend to create order">{`Can spend ${this.props.currency}`}</label>
                    </div>
                    <div className="max-purch__values">
                      <input type="number" min="0.001" step="0.001" className="max-purch__input" id="max-purch__input" name="max-purch__input" placeholder={`1.00 ${this.props.symbol}`}></input>
                    </div>
                  </div> 

                  <div className="order-life-time controls-form__item app-block__item__elem">
                    <div className="order-life-time__label">
                      <label className="order-life-time__label-caption" title="After a while to cancel order (min)">Order life time</label>
                    </div>
                    <div className="order-life-time__values">
                      <input type="number" min="1" step="1" className="order-life-time__input" id="order-life-time__input" name="order-life-time__input" placeholder="3 min"></input>
                    </div>
                  </div>
                  ...
                  <div className="take-profit app-block__item__elem">
                    ...
                    <div className="take-profit__values">   
                      <input type="number" min="0.001" step="0.01" 
                         className="takeProfit__input" id="takeProfit__input" 
                         name="takeProfit__input" placeholder="#Disabled"  
                         disabled>
                      </input>
                    </div>
                  </div>
                  ...
                  <div className="stop-loss controls-form__item app-block__item__elem">
                    ...
                    <div className="stop-loss__values">
                      <input type="number" min="0.001" step="0.01" className="stopLoss__input" id="stopLoss__input" name="stopLoss__input" placeholder="#Disabled" disabled></input>
                    </div>
                  </div> 
                   
                  <div className="use-macd controls-form__item app-block__item__elem">
                    <div className="use-macd-caption">
                      <div className="use-macd-caption__label "><label>Use MACD</label></div>
                      <div className="use-macd-caption__state "><input type="checkbox" disabled/></div>
                    </div>
                  </div> 
                        ...
                        let openedOrders = api_query("user_open_orders", {})
                          .then(res=>{
                            ...
                            if (data.hasOwnProperty('error')) {
                              ...
                              this.addLog(`Getting opened orders for ${pair} failed with error: ${data.error}. Exit`);
                            } else return data;
                          },  error => {
                                return error;
                              }
