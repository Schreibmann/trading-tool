...
class AutoTrading extends React.PureComponent {
  state = {
    trading: false,
    showConsole: false,
    consoleText: [],
        ...
        <div className="check-period controls-form__item app-block__item__elem">
          ...
          <div className="check-period__values">
            <select
              ...
              onChange={e => this.props.setAvgPricePeriod(e)}
            >
...
AutoTrading.prototype.addLog = function (text) {
  const key = Date.now();
  const node = <p key={key}>{text}</p>;
  this.setState(prevState => ({
    consoleText: [node, ...prevState.consoleText],
  }));
};
